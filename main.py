...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == 'monitor':
        ...
        await update.message.reply_text(
            'Бот оповестит вас о бесплатной приемке!',
    ...
    if api_token and warehouse_ids:
        ...
        if coefficients:
            for item in coefficients:
                ...
                date = item.get('date', None).split("T")[0]  # Обрезаем время, оставляем только дату
                ...
                box_type_name = box_type_names.get(box_type_id, 'Неизвестный тип')

                if coefficient == 0 and box_type_id == 2:
                    warehouse_name = WAREHOUSE_NAMES.get(warehouse_id)
                    if warehouse_name:
                        message = (
                            f"✅Доступная бесплатная приемка!✅\n\n"
